			...
			<div class="portlet box grey">
				<div class="portlet-title">
					...
					<div class="clearfix fr">
						...
						<%--</a>--%>
						<%--<a id="sample_editable_1_new" class="btn green" href="${path }/xx/app/console/gendata/loadGenerator">--%>
						...
						<div class="btn-group">
							...
							<ul class="dropdown-menu">
								...
								<li>
									<a  href="${path }/xx/app/console/gendata/loadGenerator?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}">新数据包</a>
								</li>
								<li>
...
<script type="text/javascript">
	...
	function generatorDataByOldPacket(){
		...
		window.location.href ="${path }/xx/app/console/gendata/loadGeneratorByoldPacket?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}&oldPacketId="+oldPacketId;
	}
	function pagehref (pageNum ,pageSize){
		window.location.href ="${path}/xx/app/console/gendata/?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}&pageNum="+pageNum+"&pageSize="+pageSize;
	}
	...
	var del = function(id,itemName){
		actionByConfirm1(itemName+"的数据包", "${path}/xx/app/console/gendata/delete/" + id+"?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}",{} ,function(data,status){
			if (data.success == true) {
				showTip("提示","删除成功", 2000);
				setTimeout(function(){window.location.href = "${path}/xx/app/console/gendata/?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}"},2000);
			}else{
	...
	var delMore = function(){
		...
		actionByConfirm1("选择的数据包", "${path}/xx/app/console/gendata/delete/" + delIds+"?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}",{} ,function(data,status){
			if (data.success == true) {
				showTip("提示","删除成功", 2000);
				setTimeout(function(){window.location.href = "${path}/xx/app/console/gendata/?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}"},2000);
			}else{
	...
	function changeStatue(id){
		$.cloudAjax({
			path : '${path}',
			url : "${path }/xx/app/console/gendata/changePacketZt/"+id,
			type : "post",
...
//					document.getElementById(id+"_shZt").innerHTML  = data.shZtValue;
	...
	function genInitDataPacket() {
		$.cloudAjax({
			path : '${path}',
			url : "${path }/xx/app/console/gendata/generator/init",
			type : "post",
			data : $("#form1").serialize(),
			dataType : "json",
			headers:{
			...
			success : function(data){
				if(data.success){
					...
					setTimeout(function(){window.location.href = "${path}/xx/app/console/gendata/?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}"},1000);
				}else{
	...
	function downZip(id) {
		window.open("${path }/xx/app/console/gendata/zip/down?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}&id="+id);
	}
</script>
</body>
</html>
