...
//		}
		var _id = node.id;
		$.ajax({
			url : "${path}/xx/app/console/bset/ajax/addOrEdit",
			type : "post",
...
//		}
	...
	var deleteTreeNode = function(){
		...
		var id = node.id;
		showPrompModal(node.name,msg,tip,"${path}/xx/app/console/bset/delete/" + id+"?dataType="+node.dataType,null, function(json){
			if(json.success == true){
				showTip("提示","删除“"+node.name+"”成功", 2000);
				setTimeout(function(){window.location.href = "${path}/xx/app/console/bset/"},2000);
		...
		<%--actionByConfirm1(itemName, "${path}/xx/app/console/bset/delete/" + id,{} ,function(data,status){--%>
