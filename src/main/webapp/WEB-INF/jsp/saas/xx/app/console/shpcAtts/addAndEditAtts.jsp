	...
	(function(){
		...
		function ajaxSubmit(){
			...
			$("#importForm").ajaxSubmit({
				url : "${path }/xx/app/console/shpcAtts/ajax/uploadAtt?shpcId=${shpcId}",
				type : "post",
				headers:{
...
//				},
				success : function(json){
					if(json.code == 1){
						...
						<%--window.location.href="${path }/xx/app/console/Sha01/list?shpcId=${shpcId}";--%>
					}else if(json.code == -1){
...
//					showTip("提示","出错了,请检查网络!",500);
	...
	var del = function(id,itemName){
		actionByConfirm1(itemName, "${path}/xx/app/console/shpcAtts/delete/" + id+"?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}",{} ,function(data,status){
	...
	function fileDown(id) {
		window.open("${path }/xx/app/console/shpcAtts/ajax/down?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}&id="+id);
	}
</script>