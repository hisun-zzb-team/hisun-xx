...
<body class="page-header-fixed white">
	<div class="row-fluid mb10">
		...
		<div class="fr">
			...
			<a type="button" class="btn" style="height: 20px;" href="${path }/xx/app/console/gendata/loadGenerator?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}" ><i class="icon-refresh"></i>刷新</a>
			<a class="btn" href="${path}/xx/app/console/gendata/?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}"><i class="icon-undo"></i>返回</a>
		</div>
	</div>
<form action="${path }/xx/app/console/gendata/generator?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}" class="form-horizontal" id="form1" method="post">
...
//			var winopenRef = window.open("","_blank");
			$.cloudAjax({
				path : '${path}',
				url : "${path }/xx/app/console/gendata/generatorByoldPacket",
				type : "post",
				...
				success : function(data){
					if(data.success){
						//winopenRef.location="${path }/xx/app/console/gendata/zip/down?id="+data.gendataId;
