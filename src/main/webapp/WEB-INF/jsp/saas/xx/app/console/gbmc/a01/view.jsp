...
<div class="xwbmain">
    ...
    <div class="mainone">
        ...
        <div class="mainoneleft">
            <img class="imgtp" width="180" height="200" src="${path}/xx/app/console/gbmc/a01/${gbMcA01Vo.id}/photo?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}" />
        </div>
        <div class="clearfix fr">
            <a class="btn red" href="javascript:del('${gbMcA01Vo.xm }')"><i class=" icon-remove-sign"></i>删除</a>
            <a class="btn" href="${path }/xx/app/console/gbmc/a01/list?mcid=${mcid}&mcb01id=${mcb01id}"><i class="icon-undo"></i>返回</a>
        </div>
...
<script type="text/javascript">
    ...
    function gbrmspbSubmit() {
        ...
        $("#importForm").ajaxSubmit({
            url: "${path }/xx/app/console/GbMca01/gbrmspb/ajax/uploadFile?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}&gbMcA01Id=${gbMcA01Vo.id}",
            type: "post",
            headers: {
            ...
            success: function (json) {
                if (json.code == 1) {
                    ...
                    window.location.href="${path }/xx/app/console/gbmc/a01/view?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}&id=${gbMcA01Vo.id}";
                } else if (json.code == -1) {
    ...
    function gbrmspbDown() {
        window.open("${path }/xx/app/console/GbMca01/gbrmspb/ajax/down?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}&gbMcA01Id=${gbMcA01Vo.id}");
    }


     function del(itemName){
        actionByConfirm1(itemName, "${path}/xx/app/console/gbmc/a01/delete/${gbMcA01Vo.id}?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}",{} ,function(data,status){
            if (data.success == true) {
                showTip("提示","删除成功", 1000);
                setTimeout(function(){window.location.href = "${path}/xx/app/console/gbmc/a01/list?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}&mcid=${mcid}&mcb01id=${mcb01id}"},1000);
            }else{
