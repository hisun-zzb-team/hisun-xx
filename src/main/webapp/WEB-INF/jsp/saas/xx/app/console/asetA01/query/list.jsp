...
<div id="mcModal" class="modal container hide fade" tabindex="-1" data-width="400">
						...
						<c:if test="${empty queryId}">
							<div class="btn-group">
								...
								<ul class="dropdown-menu" style="left: -60px;">
									<li >
										<a href="${path }/xx/app/console/asetA01Query/add?addType=a01List&OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}">新条件</a>
									</li>
									<li class="divider"></li>
									...
									<li class="divider"></li>
									<li>
										<a href="${path }/xx/app/console/asetA01Query/queryList?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}">条件管理</a>
									</li>
								</ul>
							</div>
							<a id="sample_editable_1_new" class="btn green" href="${path }/xx/app/console/asetA01/?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}">
								按机构查询
							</a>
							...
							<c:if test="${!empty queryId}">
								...
								<c:if test="${empty queryPosition}">
									<a class="btn" href="${path }/xx/app/console/asetA01Query/?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}"><i class="icon-undo"></i>返回</a>
								</c:if>
								<c:if test="${queryPosition eq 'queryList'}">
									<a class="btn" href="${path }/xx/app/console/asetA01Query/queryList?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}"><i class="icon-undo"></i> 返回</a>
								</c:if>
							</c:if>
					...
					<div class="clearfix">
						<div class="control-group">
							<div id="query" style="float: left;">
								<form action="${path }/xx/app/console/asetA01Query/?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}" method="POST" id="searchForm" name="searchForm">
						...
						<table class="table table-striped table-bordered table-hover dataTable table-set">
							...
							<c:forEach items="${pager.datas}" var="vo">
								<tr style="text-overflow:ellipsis;">
									<td title="${vo.xm}"><a href="${path}/xx/app/console/asetA01Query/view?id=${vo.id }&queryId=${queryId}&queryPosition=${queryPosition}&OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}"><c:out value="${vo.xm}"></c:out></a></td>
										<%--<td title="${vo.xm}">--%>
	...
	<script type="text/javascript">
		...
		function pagehref (pageNum ,pageSize){
			window.location.href ="${path}/xx/app/console/asetA01Query/?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}&queryId=${queryId}&pageNum="+pageNum+"&pageSize="+pageSize;
		}
		...
		function savaAsGbmc(){
			...
			$.cloudAjax({
				path : '${path}',
				url : "${path }/xx/app/console/asetA01Query/savaAsGbmc",
				type : "post",
		...
		var del = function(id,itemName){
			actionByConfirm1(itemName, "${path}/xx/app/console/asetA01Query/delete/" + id+"?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}",{} ,function(data,status){
				if (data.success == true) {
					...
					setTimeout(function(){window.location.href = "${path}/xx/app/console/asetA01Query/?queryId=${queryId}&OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}"},2000);
				}else{
		...
		function queryByCondition(queryId){
			window.location.href = "${path}/xx/app/console/asetA01Query/?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}&queryId="+queryId;
		}
		...
		function saveCondition(){
			...
			$.cloudAjax({
				path : '${path}',
				url : "${path }/xx/app/console/asetA01Query/saveCondition",
				type : "post",
