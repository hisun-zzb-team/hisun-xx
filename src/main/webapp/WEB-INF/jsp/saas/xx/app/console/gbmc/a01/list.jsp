				...
				<div class="portlet-title">
					...
					<div class="clearfix fr">
						...
						<c:if test="${returnList eq 'b01Lis'}">
							<a class="btn" href="${path }/xx/app/console/gbmc/b01/list?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}&mcid=${mcid}"><i class="icon-undo"></i>返回</a>
						</c:if>
						<c:if test="${returnList eq 'gbmcList'}">
							<a class="btn" href="${path }/xx/app/console/gbmc/?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}"><i class="icon-undo"></i>返回</a>
						</c:if>
					</div>
				</div>
			</form>
			<div class="clearfix">
				<div class="control-group">
					<div id="query" style="float: left;">
						<form action="${path }/xx/app/console/gbmc/a01/list?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}" method="POST" id="searchForm" name="searchForm">
					...
					<table class="table table-striped table-bordered table-hover dataTable table-set">
						...
						<c:forEach items="${pager.datas}" var="vo">
							<tr style="text-overflow:ellipsis;">
								<td title="${vo.xm}"><a href="${path}/xx/app/console/gbmc/a01/view?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}&id=${vo.id }"><c:out value="${vo.xm}"></c:out></a></td>
...
<script type="text/javascript">
	...
	(function(){
		...
		function ajaxSubmit(){
			...
			$("#importForm").ajaxSubmit({
				url : "${path }/xx/app/console/gbmc/a01/ajax/execute?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}&mcb01id=${mcb01id}",
				type : "post",
				headers:{
				...
				success : function(json){
					if(json.code == 1){
						...
						window.location.href="${path}/xx/app/console/gbmc/a01/list?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}&mcb01id=${mcb01id}&mcid=${mcid}";
					}else if(json.code == -1){
		...
		function gbrmspbSubmit() {
			...
			$("#importForm").ajaxSubmit({
				url:"${path }/xx/app/console/GbMca01/gbrmspb/ajax/uploadFile?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}&gbMcA01Id="+a01Id ,
				type: "post",
				headers: {
					OWASP_CSRFTOKEN: "${sessionScope.OWASP_CSRFTOKEN}"
				},
		...
		function moreGbrmspbSubmit() {
			...
			$("#importForm").ajaxSubmit({
				url: "${path }/xx/app/console/GbMca01/gbrmspb/ajax/batch/upload?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}&mcb01id=${mcb01id}",
				type: "post",
				headers: {
					OWASP_CSRFTOKEN: "${sessionScope.OWASP_CSRFTOKEN}"
				},
				...
				success: function (json) {
					if (json.code == 1) {
						showTip("提示","上传成功",2000);
						<%--window.location.href="${path}/xx/app/console/gbmc/a01/list?mcb01id=${mcb01id}&mcid=${mcid}";--%>
					} else if (json.code == -1) {
						showTip("提示", json.message, 2000);
					} else {
						showTip("提示", "出错了,请检查网络!", 2000);
	...
	function pagehref (pageNum ,pageSize){
		<%--window.location.href ="${path}/xx/app/console/gbmc/a01/list?mcb01id=${mcb01id}&mcid=${mcid}&pageNum="+pageNum+"&pageSize="+pageSize;--%>
		$("#pageNum").val(pageNum);
	...
	var del = function(id,itemName){
		actionByConfirm1(itemName, "${path}/xx/app/console/gbmc/a01/delete/" + id+"?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}",{} ,function(data,status){
			if (data.success == true) {
				showTip("提示","删除成功", 2000);
				setTimeout(function(){window.location.href = "${path}/xx/app/console/gbmc/a01/list?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}&mcb01id=${mcb01id}&mcid=${mcid}"},2000);
			}else{
	...
	function gbrmspbDown(id) {
		window.open("${path }/xx/app/console/GbMca01/gbrmspb/ajax/down?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}&gbMcA01Id="+id);
	}
