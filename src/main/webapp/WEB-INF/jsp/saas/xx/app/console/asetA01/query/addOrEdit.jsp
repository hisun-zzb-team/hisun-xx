					...
					<div class="portlet-title">
						...
						<div class="clearfix fr">
							...
							<c:if test="${addType eq 'a01List'}">
								<a class="btn" href="${path }/xx/app/console/asetA01Query/?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}"><i class="icon-remove-sign"></i> 取消</a>
							</c:if>
							<c:if test="${empty addType}">
								<a class="btn" href="${path }/xx/app/console/asetA01Query/queryList?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}"><i class="icon-remove-sign"></i> 取消</a>
							</c:if>
...
jQuery(document).ready(function() {
	...
	var resTree;
	$(function(){
		localPost("<%=path%>/xx/app/console/bset/ajax/load/tree?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}",{"status":1}, function(data,status){
			if (status == "success") {
				//setting.data.key.url = "_url" ;
				resTree = $.fn.zTree.init($("#treeDemo"), setting, data.customTree);

				localPost("<%=path%>/xx/app/console/asetA01Query/selectB01?queryId=${vo.id}&OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}",null, function(data,status){
					if (status == "success") {
...
//					showTip("提示","排序必须为数字",2000);
		...
		if(bool){
			...
			$.cloudAjax({
				path : '${path}',
				url : "${path }/xx/app/console/asetA01Query/save",
				type : "post",
...
//						showTip("提示","操作成功",2000);
						window.location.href = "${path}/xx/app/console/asetA01Query/?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}&queryId="+data.id;
					}else{
