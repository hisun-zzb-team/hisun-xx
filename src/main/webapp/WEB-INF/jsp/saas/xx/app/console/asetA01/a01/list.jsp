			...
			<form class=""id="importForm" enctype="multipart/form-data">
				...
				<div class="portlet-title">
					...
					<div class="clearfix fr">
						...
						<a id="sample_editable_1_new" class="btn green" href="${path }/xx/app/console/asetA01Query/?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}">
						按条件查询
					</a>
						<div class="btn-group">
							<a class="btn green dropdown-toggle" data-toggle="dropdown" href="#">
			...
			<div class="clearfix">
				<div class="control-group">
					<div id="query" style="float: left;">
						<form action="${path }/xx/app/console/asetA01/ajax/list?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}" method="POST" id="searchForm" name="searchForm">
...
<script type="text/javascript">
	(function(){
		...
		function ajaxSubmit(){
			...
			$("#importForm").ajaxSubmit({
				url : "${path }/xx/app/console/asetA01/ajax/execute?b01Id=${b01Id}",
				type : "post",
				headers:{
		...
		function gbrmspbSubmit() {
			...
			$("#importForm").ajaxSubmit({
				url: "${path }/xx/app/console/GbMca01/gbrmspb/ajax/batch/upload?b01Id=${b01Id}",
				type: "post",
				headers: {
					OWASP_CSRFTOKEN: "${sessionScope.OWASP_CSRFTOKEN}"
				},
				...
				success: function (json) {
					if (json.code == 1) {
						...
						<%--window.location.href="${path}/xx/app/console/gbmc/a01/list?b01Id=${b01Id}&mcid=${mcid}";--%>
					} else if (json.code == -1) {
						showTip("提示", json.message, 2000);
					} else {
						showTip("提示", "出错了,请检查网络!", 2000);
	...
	function pagehref (pageNum ,pageSize){
		<%--window.location.href ="${path}/xx/app/console/gbmc/a01/list?b01Id=${b01Id}&mcid=${mcid}&pageNum="+pageNum+"&pageSize="+pageSize;--%>
		$.ajax({
			async:false,
			type:"POST",
			url:"${path}/xx/app/console/asetA01/ajax/list",
			dataType : "html",
			headers:{
				"OWASP_CSRFTOKEN":'${sessionScope.OWASP_CSRFTOKEN}'
			},
...
//				$("#treeId").val(nodeId);
	...
	var view = function(id){
		$.ajax({
			async:false,
			type:"POST",
			url:"${path}/xx/app/console/asetA01/ajax/view",
			dataType : "html",
			headers:{
				"OWASP_CSRFTOKEN":'${sessionScope.OWASP_CSRFTOKEN}'
			},
...
//				$("#treeId").val(nodeId);
	...
	var del = function(id,itemName){
		actionByConfirm1(itemName, "${path}/xx/app/console/asetA01/delete/" + id+"?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}",{} ,function(data,status){
			if (data.success == true) {
				showTip("提示","删除成功", 2000);
				setTimeout(function(){window.location.href = "${path}/xx/app/console/asetA01/list?b01Id=${b01Id}&mcid=${mcid}&OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}"},2000);
			}else{
...
//				$("#treeId").val(nodeId);
	...
	function exportGbrmsp(){
		$.cloudAjax({
			path : '${path}',
			url : "${path }/xx/app/console/asetA01/ajax/exportGbrmsp",
			type : "post",
