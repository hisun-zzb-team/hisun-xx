			...
			<div class="clearfix">
				<div class="control-group">
					<div id="query" style="float: left;">
						<form action="${path }/xx/app/console/bset/ajax/list" method="POST" id="searchForm" name="searchForm">
					...
					<table class="table table-striped table-bordered table-hover dataTable table-set">
						...
						<c:forEach items="${pager.datas}" var="vo">
							<tr style="text-overflow:ellipsis;">
								<%--<td title="${vo.xm}"><a href="${path}/xx/app/console/appGbcxA01/view?id=${vo.id }"><c:out value="${vo.xm}"></c:out></a></td>--%>
								<td><a href="${path }/xx/app/console/bset/addOrEditManage?dataType=0&parentId=${queryId}"><c:out value="${vo.b0101}"></c:out></a></td>
...
<script type="text/javascript">
	function importParmenter(importType){
		$.ajax({
			url : "${path}/xx/app/console/bset/ajax/importParamSet",
			type : "post",
	...
	(function(){
		...
		function ajaxSubmit(){
			...
			$("#importForm").ajaxSubmit({
				url : "${path }/xx/app/console/appGbcxA01/ajax/execute?b01Id=${b01Id}",
				type : "post",
				headers:{
					OWASP_CSRFTOKEN:"${sessionScope.OWASP_CSRFTOKEN}"
				},
	...
	function pagehref (pageNum ,pageSize){
		...
		$.ajax({
			async:false,
			type:"POST",
			url:"${path}/xx/app/console/bset/ajax/list",
			dataType : "html",
			headers:{
				"OWASP_CSRFTOKEN":'${sessionScope.OWASP_CSRFTOKEN}'
			},
...
//				$("#treeId").val(nodeId);
	...
	var view = function(id){
		$.ajax({
			async:false,
			type:"POST",
			url:"${path}/xx/app/console/appGbcxA01/ajax/view",
			dataType : "html",
			headers:{
				"OWASP_CSRFTOKEN":'${sessionScope.OWASP_CSRFTOKEN}'
			},
...
//				$("#treeId").val(nodeId);
	...
	var del = function(id,itemName){
		actionByConfirm1(itemName, "${path}/xx/app/console/appGbcxA01/delete/" + id,{} ,function(data,status){
			if (data.success == true) {
				showTip("提示","删除成功", 2000);
				setTimeout(function(){window.location.href = "${path}/xx/app/console/appGbcxA01/list?b01Id=${b01Id}&mcid=${mcid}"},2000);
			}else{
