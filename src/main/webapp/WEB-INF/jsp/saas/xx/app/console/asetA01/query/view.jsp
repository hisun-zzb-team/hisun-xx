...
<div class="xwbmain">
    ...
    <div class="mainone">
        ...
        <div class="mainoneleft">
            <img class="imgtp" width="180" height="200" src="${path}/xx/app/console/asetA01/${a01Vo.id}/photo?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}" />
        </div>
        <div class="clearfix fr">
            ...
            <a class="btn" href="${path }/xx/app/console/asetA01Query/?queryId=${queryId}&queryPosition=${queryPosition}&OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}"><i class="icon-undo"></i>返回</a>
        </div>
...
<script type="text/javascript">
    ...
    function gbrmspbSubmit() {
        ...
        $("#importForm").ajaxSubmit({
            url: "${path }/xx/app/console/GbMca01/gbrmspb/ajax/uploadFile?gbMcA01Id=${a01Vo.id}&OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}",
            type: "post",
            headers: {
            ...
            success: function (json) {
                if (json.code == 1) {
                    ...
                    window.location.href="${path }/xx/app/console/gbmc/a01/view?id=${a01Vo.id}&OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}";
                } else if (json.code == -1) {
    ...
    function fileDown() {
       ...
       // }else{
             window.open("${path }/xx/app/console/asetA01/ajax/down?id=${a01Vo.id}&OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}");
       // }
    }


     function del(itemName){
        actionByConfirm1(itemName, "${path}/xx/app/console/asetA01/delete/${a01Vo.id}?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}",{} ,function(data,status){
            if (data.success == true) {
                showTip("提示","删除成功", 1000);
                returnList();
                <%--setTimeout(function(){window.location.href = "${path}/xx/app/console/gbmc/a01/list?mcid=${mcid}&mcb01id=${mcb01id}"},1000);--%>
            }else{
