...
//				beforeDrop: beforeDrop,
		...
		$.ajax({
			async:false,
			type:"POST",
			url:"${path}/xx/app/console/bset/ajax/load/tree",
			dataType : "json",
			headers:{
...
//		}
		var _id = node.id;
		$.ajax({
			url : "${path}/xx/app/console/appGbcxB01/ajax/addOrEdit",
			type : "post",
...
//		}
	...
	var deleteTreeNode = function(){
		...
		actionByConfirm1(itemName, "${path}/xx/app/console/appGbcxB01/delete/" + id+"?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}",{} ,function(data,status){
			if (data.success == true) {
				showTip("提示","删除成功", 2000);
				setTimeout(function(){window.location.href = "${path}/xx/app/console/gbcx/?OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}"},2000);
			}else{
	...
	function loadRightPage(nodeId) {
		$.ajax({
			async:false,
			type:"POST",
			url:"${path}/xx/app/console/asetA01/ajax/manageList",
			dataType : "html",
			headers:{
				"OWASP_CSRFTOKEN":'${sessionScope.OWASP_CSRFTOKEN}'
			},
